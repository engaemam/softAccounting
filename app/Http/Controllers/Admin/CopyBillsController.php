...
//        \File::put(public_path('upload/barcodebills') . '/' . $imageName, base64_decode($image));
        ...
        foreach ($request->item_id as $k_item=>$item){
            if ($request->color[$k_item] != 20){
                foreach ($request->u_size[$request->color[$k_item]] as $k_size => $itemSize) {
                    $fire []=$itemSize;
                    $zz =round($request->u_price[$request->color[$k_item]][$k_size], 2);
                $items = Items::where('id',$item)->update(['price'=>$zz]);
            $billitems = Billitems::create([
            'bill_id' => $bill->id,
            'item_id' => $item,
            'color' => $request->color[$k_item],
            'size' =>  $itemSize,
            'quantity_b' => $request->u_quantity[$request->color[$k_item]][$k_size],
            'price_b' => round($request->u_price[$request->color[$k_item]][$k_size], 2) ,
           'total_price_b' => round($request->u_price[$request->color[$k_item]][$k_size], 2)* $request->u_quantity[$request->color[$k_item]][$k_size],
        ...
        if(!empty($request->item_id)) {
            ...
            foreach ($request->item_id as $key => $value) {
                ...
                if ($request->color[$key] != "20"){
                    foreach ($request->u_size[$request->color[$key]] as $key_size => $size_id){
                        if($Specifications = Specifications::where('item_id', $value)->where('size', $size_id)->where('color_id', $request->color[$key])->first()) {
                            $Specifications->quantity += $request->u_quantity[$request->color[$key]][$key_size];
                            $Specifications->selling_price = round($request->u_price[$request->color[$key]][$key_size], 2);
                            $Specifications->save();
                            if ($Items = Items::find($value)) {
                                $Items->quantity += $request->u_quantity[$request->color[$key]][$key_size];
                                $Items->save();
                            }
                    ...
                    if($Specifications = Specifications::where('item_id', $value)->where('size', 27)->where('color_id', $request->color[$key])->first()) {
    ...
    public function edit($id)
    {
        ...
        $billitems = Billitems::where('bill_id', $bill->id)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        $deletedRows = Billdevies::where('bill_id', $bill->id)->delete();
        $deletedRows = Billdevicesitems::where('bill_id', $bill->id)->delete();

        if (!empty($request->devices)) {
            foreach ($request->devices as $device_id => $device) {

                // single device
                $Billdevies = Billdevies::create([
                    'bill_id' => $bill->id,
                    'device_id' => $device['device_id'],
                    'quantity' => $device['device_quantity'],
                    'onedevices' => round(($device['device_total_price'] / $device['device_quantity']), 2),
                    'onedevices_egy' => round((($device['device_total_price'] / $device['device_quantity']) * ($bill->price_before_doller)), 2),

                    'total_price' => round($device['device_total_price'], 2),
                    'total_price_egy' => round($device['device_total_price'] * ($bill->price_before_doller), 2),
                ]);
                foreach ($device['device_items'] as $item_id => $item) {
                    $Billdevicesitems = Billdevicesitems::create([
                        'bill_id' => $bill->id,
                        'device_id' => $device['device_id'],
                        ...
                        'price_devices_egy' => round(($item['p'] * ($bill->price_before_doller)), 2),
                        'total_devices_egy' => round(($item['total_p'] * ($bill->price_before_doller)), 2),
...
//            'percentageTax',
    ...
    {
        $invoicesPDF = Billspdf::find($id)->delete();
        session()->flash('success', trans('admin.deleted_record'));
        return back();
    }
    // End PDF //
