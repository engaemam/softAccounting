...
@section('content')
    ...
    <div class="box">
        <div class="box-header">
            ...
            <a href="{{url('admin/returnedbills')}}" class="btn btn-primary btn-lg pull-left">رجوع</a>
        </div>
            ...
            <form class="form-horizontal" method="POST" action="{{route('returnedbills.update',[$invoices->id])}}"  enctype="multipart/form-data">
                ...
                <div class="box-body">
                    <div class="form-group">
                        <label class="col-sm-1 control-label" for="name">{{trans('invoices.invoices_number')}}</label>
                        <div class="col-sm-3">
                            <input class="form-control" name="client_id" type="hidden" readonly value="{{$invoices->id}}"><h5>{{$invoices->id}}</h5>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <input class="form-control" type="hidden" name="invoice_source_id" readonly value="{{$invoices->invoice_source_id}}"><h5>{{$invoices->invoiceSource->name}}</h5>
                        </div>
                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <table class="table table-bordered">
                                                ...
                                                <tr>
                                                    ...
                                                    <th>القيمة التي تم إرجاعها للمخزن</th>
                                                    <th>المواصفة</th>
                                                    <th>مواصفة 2</th>
                                                    <th>سعر الماده</th>
                                                    <th>الاجمالي</th>
                                                ...
                                                @foreach($invoiceitems as $k=>$invoiceitem)
                                                    <tr id="remove{{$k}} ">
                                                        ...
                                                        <td>
                                                            <input id="No1" type="number" name="quantity_b[]" readonly class="form-control" min="1" value="{{$invoiceitem->quantity_b}}" max="{{ \App\Model\Invoiceitems::where('item_id',$invoiceitem->item_id)->where('color',$invoiceitem->color)->where('size',$invoiceitem->size)->pluck('quantity_b')->first() }}" placeholder="كمية المرتجع" required>
                                                            <span>المتبقي : {{ \App\Model\Invoiceitems::where('item_id',$invoiceitem->item_id)->where('color',$invoiceitem->color)->where('size',$invoiceitem->size)->pluck('quantity_b')->first() - $invoiceitem->quantity_b }}</span>
                                                        </td>
                                                        <td>
